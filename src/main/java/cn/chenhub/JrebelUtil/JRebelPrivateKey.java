package cn.chenhub.JrebelUtil;

import org.bouncycastle.jce.provider.BouncyCastleProvider;

import java.security.GeneralSecurityException;
import java.security.KeyFactory;
import java.security.PrivateKey;
import java.security.Signature;
import java.security.spec.PKCS8EncodedKeySpec;

class JRebelPrivateKey {
    private static final String b = "";
    private static final byte[] c;
    private static final BouncyCastleProvider d;

    private static PrivateKey a() {
        final PKCS8EncodedKeySpec pkcs8EncodedKeySpec = new PKCS8EncodedKeySpec(JRebelPrivateKey.c);
        try {
            return KeyFactory.getInstance("RSA", JRebelPrivateKey.d).generatePrivate(pkcs8EncodedKeySpec);
        } catch (Exception ex) {
            ex.printStackTrace();
            return null;
        }
    }

    static byte[] a(final byte[] array) {
        try {
            final Signature instance = Signature.getInstance("SHA1withRSA", JRebelPrivateKey.d);
            instance.initSign(a());
            instance.update(array);
            return instance.sign();
        } catch (GeneralSecurityException ex) {
            throw new RuntimeException("License Server installation error 0000000F2", ex);
        }
    }

    static {
        c = ByteUtil.a("");
        d = new BouncyCastleProvider();
    }
}
